...
- 📡 **クラウド連携**: WiFi経由でGoogle Apps Scriptへ自動POST（記録データは10秒バッチ）
...
45.678             ← 換算値（大きく表示）
%                  ← 単位
#sym:<group_id> #sym:<machine_id> ← 識別子
MAC:xx:xx:xx:xx:xx:xx             ← MACアドレス
IP:192.168.x.x                    ← WiFi接続時のみ表示
2024-01-01 12:00:00               ← 現在時刻
```
...
```json
{
  ...
  "interval_sec": 10,
  "count": 2,
  "records": "[{\"timestamp\":\"2024-01-01 12:00:00\",\"voltage\":1.234,\"pressure\":45.678}]"
}
```

**HTTP POST (JSON / stable heartbeat):**
```json
{
  "group_id": "DPG",
  "machine_id": "ATOMS3-01",
